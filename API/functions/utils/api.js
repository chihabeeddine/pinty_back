...
module.exports = {
  ...
  checkUserAuthorization(roles, id, apiKey) {
    return new Promise((resolve, reject) => {
      if (!roles || roles.toLowerCase() == 'all') roles = config.static.roles;

      if (id && apiKey) {
        if (id.match(/^[0-9a-fA-F]{24}$/)) {
          // Check good ObjectID
          Server.fn.dbMethods.user
            .hasPerm(roles, id, apiKey)
            .then(perm => {
              if (perm.user) perm.user._id = perm.user._id.toString();
              if (perm.canPass) resolve(perm.user);
              else reject(Server.fn.api.jsonError(403, 'Forbidden'));
            })
            .catch(err => reject(Server.fn.api.jsonError(500, `'hasPerm()' Database error`, err)));
        } else return reject(Server.fn.api.jsonError(400, 'Bad pinty ID'));
      } else return reject(Server.fn.api.jsonError(400, 'Missing pinty ID or pinty key'));
    });
  },

  regexCaseAccentInsensitive(searchString) {
    const accented = {
      A:
